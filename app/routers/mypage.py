...
def mypage_dashboard(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse("mypage/dashboard.html", {"request": request, "user": user, **dashboard_data})
...
def mypage_orders(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse("mypage/orders.html", {"request": request, "user": user, "orders": orders})
...
def mypage_points(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse("mypage/points.html", {"request": request, "user": user, "points": points})
...
def mypage_reviews(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse("mypage/reviews.html", {"request": request, "user": user, "reviews": reviews})

# SSR: 계정/알림 설정
@router.get("/settings", response_class=HTMLResponse)
def mypage_settings(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return templates.TemplateResponse("mypage/settings.html", {"request": request, "user": user})
...
def mypage_purchases(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse("mypage/purchases.html", {"request": request, "user": user, "purchases": purchases})
